...
public class greed_event_info : MonoBehaviour
{
    ...
    void text_change()
    {
        ...
        if (new_card != null)
        {
            ...
            {
                num = num + calculator.x_in_y_percent(num, player.inflation);
            }
            else if (player.deflation > 0)
            {
                num = num - calculator.x_in_y_percent(num, player.deflation);
            }

            if(player.cost_reduce_buff > 0)
            {
                num = num - calculator.x_in_y_percent(num, player.cost_reduce_buff);
            }
